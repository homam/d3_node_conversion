...
right: 30,
bottom: 30,
left: 40}, width = 830 - margin.left - margin.right, height = 500 - margin.top - margin.bottom;
...
var directArea = d3.svg.area().interpolate("basis").x(function (d) {
    return xScale(d.day);
}).y0(height).y1(function (d) {
    return yScale(d.Direct);
});
d3.csv('/growth/du_bbay.csv', function (raw) {
    ...
    var svg = d3.select("body").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").datum(data);
    ...
    svg.append("g").attr("class", "y axis").call(yAxis).append("text").attr("transform", "rotate(-40)").attr("y", 15).attr("dy", ".81em").style("text-anchor", "end").text("Visits");
});
