...
d3.csv("/costrev/costrev.csv", function (rawData) {
    ...
    svgs.call(_svgs => console.log(_svgs));
        

    var country = rawData[2];
    var data = ['Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map((date,i) => {
        return {
   ...
   x.domain(d3.extent(data, d => d.date))
    ...
    y.domain([
      d3.min(data, d => Math.min(d.cost, d.rev)),
      d3.max(data, d => Math.max(d.cost, d.rev))
    ]);
 ...
 svgG.append("path")
  ...
  .attr("d", area.y0(d => y(d.rev)));
    ...
    svgG.append("g")
        .attr("class", "y axis")
        .call(() => { console.log("call",arguments); return yAxis.apply(this, arguments) })
        .append("text")
