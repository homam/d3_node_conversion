...
var yConvAxis = d3.svg.axis().scale(yConvScale).orient("right");
...
d3.csv("/wapp155/wapp155.csv", function (data) {
    ...
    svgG.datum(data);
    svgG.append("path").attr("class", "line").attr("d", visitsLine);
    svgG.append("path").attr("class", "line conversion").attr("d", convLine);
    svgG.append("g").attr("class", "x axis").attr("transform", "translate(0," + height + ")").call(xAxis);
    svgG.append("g").attr("class", "y axis").call(yVisitsAxis).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".71em").style("text-anchor", "end").text("Visits");
    svgG.append("g").attr("class", "y axis conversion").attr('transform', 'translate(' + (width) + ',0)').call(yConvAxis).append("text").attr("transform", "rotate(-90)").attr("y", -10).attr("dy", ".71em").style("text-anchor", "end").text("Conversion");
});
